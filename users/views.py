...
SCHED_CHOICES = {
    "sevenToHalf": "7:00 am - 7:30 am",
    "halfToEight": "7:30 am - 8:00 am",
    "eightToHalf": "8:00 am - 8:30 am",
    "halfToNine": "8:30 am - 9:00 am",
    "nineToHalf": "9:00 am - 9:30 am",
    "halfToTen": "9:30 am - 10:00 am",
    "tenToHalf": "10:00 am - 10:30 am",
    "halfToEleven": "10:30 am - 11:00 am",
    "elevenToHalf": "11:00 am - 11:30 am",
    "halfToTwelve": "11:30 am - 12:00 nn",
    "twelveToHalf": "12:00 nn - 12:30 pm",
    "halfToThirteen": "12:30 pm - 1:00 pm",
    "thirteenToHalf": "1:00 pm - 1:30 pm",
    "halfToFourteen": "1:30 pm - 2:00 pm",
    "fourteenToHalf": "2:00 pm - 2:30 pm",
    "halfToFifteen": "2:30 pm - 3:00 pm",
    "fifteenToHalf": "3:00 pm - 3:30 pm",
    "halfToSixteen": "3:30 pm - 4:00 pm",
    "sixteenToHalf": "4:00 pm - 4:30 pm",
    "halfToSevenTeen": "4:30 pm - 5:00 pm",
    "seventeenToHalf": "5:00 pm - 5:30 pm",
    "halfToEighteen": "5:30 pm - 6:00 pm",
    "eighteenToHalf": "6:00 pm - 6:30 pm",
    "halfToNineteen": "6:30 pm - 7:00 pms",
}
...
def register_tutor(request):
    if request.method == "POST":
        ...
        return render(
            request, "users/tutor/registertutor.html", {"picture_form": picture_form}
        )
    else:
    ...
    if request.method == "POST":
        try:
            ...
            if user is not None:
                ...
                messages.error(
                    request, "Please enter a valid username-password combination"
                )
...
def home(request):
    ...
    if user.is_tutee == True:
        ...
        if Payment.objects.filter(is_paid=False, tutee=user).exists():
            messages.error(
                request, "You have pending payments. Please pay these first."
            )
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if len(sched) < int(new_hours) * 2:
                    messages.error(
                        request, "Schedule set is not sufficient for hours requested."
                    )
                ...
                new_start = datetime.datetime.strptime(
                    new_start, settings.TIME_INPUT_FORMATS[0]
                ).time()
                ...
                try:
                    for _date in new_dates:
                        new_sessions = Sessions.objects.create(
                            user=user,
                            grade=new_grade,
                    ...
                    for y in subjects:
                        if x.user == y.user:
                            req = Requests.objects.create(
                                user=x.user, session=new_sessions
                            )
                ...
                session_num = len(Sessions_Accepted.objects.filter(tutee=user))
                messages.error(
                    request, "Error please input valid values for each field"
                )
                template = "An exception of type {0} occurred. Arguments:\n{1!r}"
                message = template.format(type(ex).__name__, ex.args)
                print(message)
        ...
        return render(
            request,
            "users/tutee/tutee_home.html",
            {"current_user": user2, "credit_user": user, "form": form},
        )
    else:
        ...
        for _request in _requests:
            ...
            _request.sessions = sorted(
                Sessions.objects.filter(code=_request.session.code),
                key=lambda x: x.session_date,
            )

        return render(
            request,
            "users/tutor/tutor_home.html",
            {"session_group": _requests, "credit_user": user},
        )
...
def upcoming(request):
    ...
    if user.is_tutee == True:
        sessions = sorted(
            Sessions_Accepted.objects.filter(tutee=user),
            key=lambda x: x.session.session_date,
        )
        return render(
            request, "users/tutee/upcoming_tutee.html", {"session_group": sessions}
        )
    else:
        sessions = sorted(
            Sessions_Accepted.objects.filter(tutor=user),
            key=lambda x: x.session.session_date,
        )
        count = 1
        tutee_now = []
        ...
        context = {"session_group": sessions, "credit_user": credit_user}
        if tutee_now is not None:
            context["cellnum"] = tutee_now

        return render(
            request, "users/tutor/upcoming_tutor.html", context, {"count": count}
        )
...
def pending(request):
    ...
    return render(
        request,
        "users/tutee/pending.html",
        {"session_group": sessions, "credit_user": user},
    )
...
def accept_tutee(request, session_id):
    if request.method == "POST":
        ...
        for _session in chosen_sessions:
            accepted_session = Sessions_Accepted.objects.create(
                session=_session,
                tutor=user,
                tutee=tutee,
                session_mutual=new_mutual_sched,
            )
...
def complete_session(request, session_id):
    ...
    if request.method == "POST":
        try:
            ...
            new_time_start = datetime.datetime.strptime(
                new_time_start, settings.TIME_INPUT_FORMATS[0]
            ).time()
            new_time_end = datetime.datetime.strptime(
                new_time_end, settings.TIME_INPUT_FORMATS[0]
            ).time()

            messages.success(
                request,
                f"Session has been completed! Please have your tutor confirm the session.",
            )
            ...
            messages.error(
                request,
                "Error: please input valid values for each field: {}".format(
                    ex.args[0]
                ),
            )
    ...
    return render(
        request,
        "users/tutor/complete_session.html",
        {"session": done, "credit_user": user},
    )
...
def unconfirmed(request):
    ...
    context = {"session_group": sessions, "credit_user": user}
...
def unconfirmed_edit(request, session_id):
    ...
    if request.method == "POST":
        try:
            ...
            if (
                new_session_date is None
                or new_time_start is None
                or new_time_end is None
            ):
                raise Exception("Unfilled form fields")
            ...
            messages.success(
                request,
                f"Session has been edited! Please have your tutor confirm the session.",
            )
...
def unconfirmed_status(request, session_id, status):
    ...
    if status == "confirm":
        ...
        messages.success(
            request, "Session has been confirmed! Please pay the necessary amount."
        )
        ...
        messages.info(
            request,
            "Session has been declined. Please wait until your tutor has corrected the details.",
        )
...
def transactions(request):
    ...
    amount_paid = transaction_group.filter(payment__is_paid=False).aggregate(
        Sum("amount")
    )["amount__sum"]
    ...
    if request.method == "POST":
        try:
            ...
            transaction = Transaction.objects.create(
                user=user, payment=current_payment, amount=amount
            )
            amount_paid_current = Transaction.objects.filter(
                payment=current_payment
            ).aggregate(current_amount=Sum("amount"))["current_amount"]
            ...
            print(message)
            messages.error(
                request, "Error: Something went wrong. Please check your inputs again."
            )
...
def history(request):
    ...
    if user.is_tutee == True:
        sessions = Sessions_Ended.objects.filter(
            final=True, unconfirmed=False, user=user
        )
...
def profile(request):
    ...
    return render(
        request,
        "users/profile.html",
        {"current_user": user2, "location_user": loc, "picture_form": picture_form},
    )
        ...
        try:
            if user.is_tutee == True:
                ...
                rate = Rating.objects.create(
                    stars=new_stars, user=user, comments=new_comments
                )
            return render("home")
        except Exception as ex:
            template = "An exception of type {0} occurred. Arguments:\n{1!r}"
