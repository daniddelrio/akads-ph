...
def create_token(number, exp_month, exp_year, cvc):
    ...
    res = requests.post(
        url, 
        json=data,
        auth=HTTPBasicAuth(config('PAYMONGO_PUBLIC_KEY'), '')
    )
    token_data = res.json()
    return token_data
...
def create_payment(token_id, minutes, tutor_name, date):
    ...
    data = {
        'data' : {
            'attributes': {
                ...
                'amount': amount*100,
                'currency': CURRENCY,
                'description': 'Payment for your tutoring session on {} with {}'.format(date, tutor_name),
                'source': {
                    'id': token_id,
                    'type': 'token'
                }
    ...
    res = requests.post(
        url, 
        json=data,
        auth=HTTPBasicAuth(config('PAYMONGO_SECRET_KEY'), '')
    )
